...
(function () {
  ...
  function isLoanNumberAllowed(loanNumber) {
    if (!isStoredNumbersSetAvailable()) {
      console.log(
        `storedNumbersSet not available, assuming loan ${loanNumber} is allowed`
      );
    ...
    if (window.storedNumbersSet instanceof Set) {
      ...
      console.log(
        `Checking loan ${loanNumber} against Object values: ${isAllowed}`
      );
    }
  ...
  function extractBrandsData() {
    ...
    try {
      ...
      if (window.brandsData && Array.isArray(window.brandsData)) {
        debugLog(
          `Found global brandsData with ${window.brandsData.length} brands from datasource.js`
        );
        ...
        window.brandsData.forEach((brand) => {
          if (
            brand.code &&
            brand.loanNumbers &&
            Array.isArray(brand.loanNumbers)
          ) {
    ...
    const customDropdowns = document.querySelectorAll(
      ".brand-dropdown, .custom-dropdown"
    );
    ...
    debugLog(
      `Analyzing ${rows.length} table rows for brand-loan relationships`
    );
    ...
    rows.forEach((row) => {
      ...
      if (rowBrandCode && rowLoanNumbers.length > 0) {
        ...
        cells.forEach((cell) => {
          ...
          if (extractedBrandCode && text.length <= 15) {
            // Brand codes are usually in short cells
    ...
    debugLog(
      `Found ${rowsWithBrandAndLoan} rows with both brand code and loan numbers`
    );
    ...
    debugLog(
      `Analyzing ${brandOptions.length} dropdown options for brand codes`
    );
    ...
    const elements = document.querySelectorAll(
      "div, span, p, td, li, a, h1, h2, h3, h4, h5, h6, label"
    );
    ...
    debugLog(
      `Found ${elementsWithBrandAndLoan} page elements with both brand code and loan numbers`
    );

    // STEP 7: Look for brand codes in proximity to loan numbers
    if (
      brandsMap.size === 0 ||
      ![...brandsMap.values()].some((brand) => brand.loanNumbers.length > 0)
    ) {
      ...
      const loanElements = textElements.filter(
        (el) => extractLoanNumbers(el.textContent).length > 0
      );
    ...
    if (window.messageThreadsData && Array.isArray(window.messageThreadsData)) {
      debugLog(
        `Found messageThreadsData with ${window.messageThreadsData.length} threads`
      );

      // Extract loan numbers from message threads
      const threadLoanNumbers = window.messageThreadsData
        .map((thread) => thread.loanNumber)
        .filter(Boolean);

      // If we have brands but no loan numbers, assign thread loan numbers to brands
      if (brandsMap.size > 0 && threadLoanNumbers.length > 0) {
        debugLog(
          `Assigning ${threadLoanNumbers.length} thread loan numbers to brands`
        );
  ...
  function brandHasAllowedLoans(brandCode) {
    try {
      if (!isStoredNumbersSetAvailable()) {
        debugLog(
          `No storedNumbersSet available, allowing all brands by default`
        );
        return true; // If we can't verify, assume it's allowed
      }
      ...
      try {
        ...
        if (!brandsData || !Array.isArray(brandsData)) {
          debugLog(
            `No valid brands data available, allowing brand ${brandCode} by default`
          );
          return true;
        }
        ...
        if (brand && brand.loanNumbers && brand.loanNumbers.length > 0) {
          debugLog(
            `Brand ${brandCode} has ${brand.loanNumbers.length} loan numbers in our data`
          );
          ...
          debugLog(
            `Brand ${brandCode} has no allowed loans in our extracted data`
          );
        } else {
          debugLog(
            `Brand ${brandCode} not found in extracted data or has no loan numbers, searching page...`
          );
        }
      } catch (e) {
        debugLog(
          `Error checking brand data: ${e.message}, allowing brand ${brandCode} by default`
        );
        return true;
      }
      ...
      try {
        ...
        for (const element of elements) {
          ...
          for (const loanNumber of loanNumbers) {
            if (isLoanNumberAllowed(loanNumber)) {
              debugLog(
                `Brand ${brandCode} found with allowed loan ${loanNumber} in page element`
              );
              return true;
            }
          }
        ...
        debugLog(
          `Brand ${brandCode} search complete: found in ${foundElements} elements with ${foundLoanNumbers} loan numbers, none allowed`
        );
      } catch (e) {
        debugLog(
          `Error searching page for brand: ${e.message}, allowing brand ${brandCode} by default`
        );
        return true;
      }
      ...
      try {
        if (window.storedNumbersSet) {
          if (
            window.storedNumbersSet instanceof Set &&
            window.storedNumbersSet.has(brandCode)
          ) {
            debugLog(`Brand code ${brandCode} is directly in storedNumbersSet`);
            return true;
          } else if (
            Array.isArray(window.storedNumbersSet) &&
            window.storedNumbersSet.includes(brandCode)
          ) {
            debugLog(
              `Brand code ${brandCode} is directly in storedNumbersSet array`
            );
            return true;
          } else if (
            typeof window.storedNumbersSet === "object" &&
            Object.values(window.storedNumbersSet).includes(brandCode)
          ) {
            debugLog(
              `Brand code ${brandCode} is directly in storedNumbersSet object values`
            );
            return true;
          }
        }
        ...
        debugLog(
          `Error checking storedNumbersSet: ${e.message}, allowing brand ${brandCode} by default`
        );
        return true;
      }
  ...
  function shouldHideElement(element) {
    ...
    if (isBrandOption) {
      ...
      if (brandCode) {
        ...
        if (!hasAllowedLoans) {
          debugLog(
            `Filtering out brand option: ${text} (${brandCode}) - no allowed loans`
          );
          return true;
        } else {
          debugLog(
            `Keeping brand option: ${text} (${brandCode}) - has allowed loans`
          );
          return false;
        }
      }
    ...
    for (const loanNumber of potentialLoanNumbers) {
      if (isLoanNumberAllowed(loanNumber)) {
        debugLog(
          `Element contains allowed loan: ${loanNumber}, keeping visible`
        );
    ...
    debugLog(
      `Filtering out element with loans: ${potentialLoanNumbers.join(", ")}`
    );
    return true;
  }
  ...
  function processBrandDropdowns() {
    ...
    brandSelects.forEach((select) => {
      ...
      if (isBrandDropdown) {
        debugLog(
          `Found brand dropdown: ${placeholder || labelElement?.textContent || "unnamed"
          }`
        );
  ...
  function processCustomBrandDropdowns() {
      ...
      brandDropdowns.forEach((dropdown) => {
        ...
        items.forEach((item) => {
          ...
          if (!brandHasAllowedLoans(brandCode)) {
            ...
            debugLog(
              `Hiding custom dropdown item: ${text} (${brandCode}) - no allowed loans`
            );
          } else {
            debugLog(
              `Keeping custom dropdown item: ${text} (${brandCode}) - has allowed loans`
            );
    ...
    const selectedBrandElement = document.querySelector(
      ".brand-dropdown .selected-option"
    );
        ...
        mutations.forEach((mutation) => {
          if (
            ...
            setTimeout(() => {
              ...
              if (typeof window.applyFilters === "function") {
                debugLog(
                  "Triggering applyFilters after brand selection change"
                );
                window.applyFilters();
              } else if (typeof window.searchMessages === "function") {
                debugLog(
                  "Triggering searchMessages after brand selection change"
                );
                window.searchMessages();
              } else if (typeof window.filterMessages === "function") {
                debugLog(
                  "Triggering filterMessages after brand selection change"
                );
                window.filterMessages();
              }
            }, 100);
  ...
  function processBrandOptions() {
    ...
    panels.forEach((panel) => {
      ...
      isBrandDropdown =
        brandCodesFound > 0 && brandCodesFound / options.length > 0.3;
      ...
      if (isBrandDropdown) {
        debugLog(
          `Identified brand dropdown with ${brandCodesFound} brand codes`
        );
        ...
        options.forEach((option) => {
          ...
          if (!brandCode) {
            ...
            if (loanNumbers.length > 0) {
              ...
              if (!hasAllowedLoan) {
                ...
                debugLog(
                  `Hiding option with no brand code but disallowed loans: ${text}`
                );
              }
            }
            return;
          ...
          if (!brandHasAllowedLoans(brandCode)) {
            ...
            debugLog(
              `Hiding brand option: ${text} (${brandCode}) - no allowed loans`
            );
  ...
  function processAllElements() {
    if (!isStoredNumbersSetAvailable()) {
      console.warn(
        "[LoanFilter] storedNumbersSet is not available yet. Waiting..."
      );
      return;
    }
    ...
    debugLog(
      `Found ${matElements.length} Angular Material elements to process`
    );
    ...
    elements.forEach((el) => {
      ...
      if (isBrandOption) {
        ...
        if (brandCode && !brandHasAllowedLoans(brandCode)) {
          ...
          const uniqueClass = `loan-filter-hide-${Math.random()
            .toString(36)
            .substring(2, 10)}`;
      ...
      Object.defineProperty(window, "initializeDropdowns", {
        ...
        set: function (newFunc) {
          ...
          window._initializeDropdownsWrapper = function () {
            ...
            debugLog(
              "initializeDropdowns called, processing brand dropdowns..."
            );
            setTimeout(() => {
  ...
  function showNotProvisionedAlert(loanNumber) {
    ...
    iconDiv.style.cssText = "font-size: 24px; margin-right: 16px;";
    ...
    titleDiv.style.cssText =
      "font-weight: bold; font-size: 16px; margin-bottom: 8px; color: #f44336;";
    titleDiv.textContent = "Loan is not provisioned to the user";

    const messageDiv = document.createElement("div");
    messageDiv.style.cssText = "color: #333; margin-bottom: 8px;";
    messageDiv.textContent = "Loan number ";
    const strongElement = document.createElement("strong");
    strongElement.textContent = loanNumber;
    messageDiv.appendChild(strongElement);
    messageDiv.appendChild(
      document.createTextNode(" is not provisioned to the current user.")
    );

    const closeButton = document.createElement("button");
    closeButton.style.cssText =
      "background: none; border: none; font-size: 20px; cursor: pointer; color: #999; padding: 0; margin-left: 8px;";
    closeButton.textContent = "×";
    closeButton.onclick = () => removeNotProvisionedAlert(true);
    ...
    if (tableBody) {
      ...
      const iconDiv = document.createElement("div");
      iconDiv.style.cssText = "margin-bottom: 10px;";
      iconDiv.textContent = "⚠️";

      const messageDiv = document.createElement("div");
      messageDiv.textContent = "Loan is not provisioned to the user";

      const loanNumberDiv = document.createElement("div");
      loanNumberDiv.style.cssText = "font-size: 14px; margin-top: 5px;";
      loanNumberDiv.textContent = `Loan number: ${loanNumber}`;

      mainDiv.appendChild(iconDiv);
      mainDiv.appendChild(messageDiv);
      mainDiv.appendChild(loanNumberDiv);
  ...
  function updateTableMessage(loanNumber) {
    ...
    const mainDiv = document.createElement("div");
    mainDiv.style.cssText =
      "color: #f44336; font-weight: bold; font-size: 16px; background-color: #fff3f3; border: 1px solid #ffcdd2; padding: 20px; border-radius: 4px;";
    ...
    if (!tableBody._notProvisionedObserver) {
      const observer = new MutationObserver((mutations) => {
        if (
          window._showingNotProvisionedMessage &&
          window._restrictedLoanNumber === loanNumber &&
          (!tableBody.textContent.includes("Loan is not provisioned") ||
            !tableBody.textContent.includes(loanNumber))
        ) {
          updateTableMessage(loanNumber);
        }
      });
    ...
    if (rows.length === 1) {
      ...
      if (cell && cell.textContent.includes("No message threads found")) {
        ...
        if (loanNumber && !isLoanNumberAllowed(loanNumber)) {
          debugLog(
            `Found "No message threads found" for restricted loan: ${loanNumber}, showing not provisioned alert`
          );
  ...
  function patchApplyFilters() {
    // Store the original function
    if (
      typeof window.originalApplyFilters === "undefined" &&
        ...
        if (loanNumber && !isLoanNumberAllowed(loanNumber)) {
          setTimeout(() => {
            if (!document.getElementById("loan-not-provisioned-alert")) {
              debugLog(
                `Re-applying not provisioned alert for ${loanNumber} after delay`
              );
              showNotProvisionedAlert(loanNumber);
            }
          }, 500);
  ...
  function preserveSortOrder(threads) {
    ...
    const sortHeaders = document.querySelectorAll(
      "th.sorted, th.sort-asc, th.sort-desc, th.mat-sort-header-sorted"
    );
  ...
  function patchPopulateMessageThreads() {
    if (
      typeof window.originalPopulateMessageThreads === "undefined" &&
      ...
      window.populateMessageThreads = function (filteredThreads = null) {
        ...
        if (
          ...
          debugLog(
            `populateMessageThreads called but we're already showing a not provisioned alert for ${window._restrictedLoanNumber}`
          );
          ...
          if (tableBody) {
            ...
            const mainDiv = document.createElement("div");
            mainDiv.style.cssText =
              "color: #f44336; font-weight: bold; padding: 20px; font-size: 16px; background-color: #fff3f3; border: 1px solid #ffcdd2; text-align: center;";
            ...
            const loanNumberDiv = document.createElement("div");
            loanNumberDiv.style.cssText =
              "font-size: 14px; margin-top: 5px;";
            loanNumberDiv.textContent = `Loan number: ${window._restrictedLoanNumber}`;

            mainDiv.appendChild(iconDiv);
            mainDiv.appendChild(messageDiv);
            mainDiv.appendChild(loanNumberDiv);

            tableBody.appendChild(mainDiv);
          }
        ...
        debugLog(
          `populateMessageThreads called with ${threads ? threads.length : 0
          } threads, loan number: ${loanNumber}`
        );

        // DIRECT CHECK: If there's a loan number and it's not allowed, show the "not provisioned" alert
        if (loanNumber && !isLoanNumberAllowed(loanNumber)) {
          debugLog(
            `Loan ${loanNumber} is not allowed, showing not provisioned alert`
          );
        ...
        if (
          ...
          debugLog(
            `Filtered message threads: ${allowedThreads.length} of ${threads.length} allowed`
          );

          // Apply field-based filtering if we have search fields
          const searchForm = document.querySelector(
            "form.search-form, .filter-form, .search-container"
          );
          if (searchForm) {
        ...
        if (loanNumber) {
          ...
          if (tableBody) {
            ...
            if (rows.length === 1) {
              ...
              if (
                cell &&
                cell.textContent.includes("No message threads found")
              ) {
                if (!isLoanNumberAllowed(loanNumber)) {
                  debugLog(
                    `Found "No message threads found" for loan ${loanNumber}, showing not provisioned alert`
                  );
                  window._showingNotProvisionedMessage = true;
                  window._restrictedLoanNumber = loanNumber;
  ...
  function addSearchEventListeners() {
    ...
    if (applyFiltersButton) {
      applyFiltersButton.addEventListener(
        ...
        function (event) {
          const loanNumberInput = document.getElementById("loanNumberFilter");
          const loanNumber = loanNumberInput
            ? loanNumberInput.value.trim()
            : "";

          if (loanNumber && !isLoanNumberAllowed(loanNumber)) {
            debugLog(
              `Apply Filters clicked for restricted loan: ${loanNumber}`
            );
        ...
        true
      ); // Use capturing to intercept the event before the original handler
    }
  }
  ...
  function setupSearchMutationObserver() {
      ...
      for (const mutation of mutations) {
        if (
          ...
          if (
            ...
            debugLog(
              `Mutation observer: Not provisioned alert was removed, restoring it for ${window._restrictedLoanNumber}`
            );
            ...
            if (tableBody) {
              ...
              if (rows.length > 1) {
                debugLog(
                  `Mutation observer: Processing ${rows.length} table rows`
                );
                ...
                rows.forEach((row) => {
                  ...
                  if (shouldHideElement(row)) {
                    ...
                    debugLog(
                      "Hiding table row:",
                      row.textContent.substring(0, 50) + "..."
                    );
                  }
                });
              }
    ...
    const refreshButton = document.querySelector(
      'button[title="Refresh"], button.refresh-button, button.btn-refresh'
    );
    if (refreshButton) {
  ...
  function ensureNotProvisionedAlertVisible() {
    ...
    if (!hasRestrictedAlert) {
      debugLog(
        `Periodic check: Not provisioned alert was removed, restoring it for ${window._restrictedLoanNumber}`
      );

      // Create a new alert with a higher z-index and make it more persistent
      const alertContainer = showNotProvisionedAlert(
        window._restrictedLoanNumber
      );
  ...
  function filterMessageThreadsByFields(threads) {
    ...
    const deletedMessagesFilter = document.getElementById(
    ...
    if (brandFilter) {
      ...
      if (selectedBrand === "All Brands") {
        ...
        debugLog(
          `Extracted brand code from selected brand: ${selectedBrandCode}`
        );
      }
    }

    debugLog(
      `Selected brand filter: ${selectedBrand || "All Brands"} (Code: ${selectedBrandCode || "None"
      })`
    );
    ...
    const filteredThreads = threads.filter((thread) => {
      ...
      if (
        ...
        queueFilter.value &&
        thread.queue &&
        thread.queue !== queueFilter.value &&
      ...
      if (
        ...
        statusFilter.value &&
        thread.status &&
        thread.status !== statusFilter.value &&
      ...
      if (
        deletedMessagesFilter &&
        deletedMessagesFilter.checked === true &&
        thread.deleted !== true
      ) {
        return false;
      }
    ...
    debugLog(
      `Filtered threads: ${filteredThreads.length} of ${threads.length} passed all filters`
    );
  ...
  function patchSearchFunction() {
    ...
    if (
      typeof window.originalSearchMessages === "undefined" &&
    ...
    if (
      typeof window.originalFilterMessages === "undefined" &&
  ...
  function patchRefreshFunction() {
    ...
    if (
      typeof window.originalRefreshMessages === "undefined" &&
  ...
  function initializeSearchResults() {
    ...
    return {
      searchObserver,
      checkInterval,
      ensureVisibleInterval,
      backupEnsureVisibleInterval,
    };
  }
  ...
  function handleRefreshButtonClick() {
    const refreshButton = document.querySelector(
      'button[title="Refresh"], button.refresh-button, button.btn-refresh, button[aria-label="Refresh"]'
    );
  ...
  const { searchObserver, checkInterval, ensureVisibleInterval } =
  ...
  window.checkFilteringFunctionality = function () {
    ...
    if (!isStoredNumbersSetAvailable()) {
      console.warn(
        "[LoanFilter] storedNumbersSet is not available - filtering will not work properly"
      );
    } else {
      console.log(
        "[LoanFilter] storedNumbersSet is available with " +
        (window.storedNumbersSet instanceof Set
          ? window.storedNumbersSet.size
          : Array.isArray(window.storedNumbersSet)
            ? window.storedNumbersSet.length
            : Object.keys(window.storedNumbersSet).length) +
        " entries"
      );
    }
    ...
    console.log(
      ...
      (window.originalPopulateMessageThreads ? "PATCHED" : "NOT PATCHED")
    );
    console.log(
      "- applyFilters: " +
      (window.originalApplyFilters ? "PATCHED" : "NOT PATCHED")
    );
    console.log(
      "- searchMessages: " +
      (window.originalSearchMessages ? "PATCHED" : "NOT PATCHED")
    );
    console.log(
      "- filterMessages: " +
      (window.originalFilterMessages ? "PATCHED" : "NOT PATCHED")
    );
    console.log(
      "- refreshMessages: " +
      (window.originalRefreshMessages ? "PATCHED" : "NOT PATCHED")
    );
    ...
    console.log(
      "[LoanFilter] Found " + brandDropdowns.length + " brand dropdowns"
    );
    ...
    if (searchForm) {
      const filterFields = searchForm.querySelectorAll("input, select");
      console.log(
        "[LoanFilter] Found search form with " +
        filterFields.length +
        " filter fields"
      );
    } else {
    ...
    console.log(
      "[LoanFilter] Refresh button: " + (refreshButton ? "FOUND" : "NOT FOUND")
    );
  ...
  window.checkBrandFiltering = function () {
    ...
    brandsData.forEach((brand) => {
      ...
      console.log(
        `[LoanFilter] Brand ${brand.code}: ${hasAllowedLoans ? "ALLOWED" : "FILTERED"
        } (${brand.loanNumbers.length} loan numbers)`
      );
    });
  ...
  console.log(
    "[LoanFilter] Combined script ready - use window.checkLoanFiltering() to debug"
  );

  // Helper function to safely clear all children of an element
