...
async function fetchAndStoreNumbers() {
  ...
  try {
    ...
    try {
      ...
      try {
        brandData = JSON.parse(textData);
        console.log(`Parsed JSON data with ${brandData.length} brand records`);
      } catch (jsonError) {
        // If not valid JSON, try to parse as CSV
        console.log("Not valid JSON, trying to parse as CSV");
        const brandIds = textData.trim().split(/\r?\n/); // Handle different line endings

        // Convert to brand objects with IDs
        brandData = brandIds
          .filter((id) => id && id.trim())
          .map((id) => ({ id: id.trim() }));
        console.log(`Parsed ${brandData.length} brand IDs from CSV`);
      }
    } catch (downloadError) {
      console.log(
...
async function storeData(data) {
  console.log(`Storing ${data.length} brand records...`);
  await sharedStorage.clearStore(STORE_NAME);

  // Store the full brand objects in memory
  storedNumbersSet = data;
...
async function notifyAndRequestData() {
  ...
  // Request brands data from any existing tabs
  channel.postMessage({
...
channel.onmessage = async (event) => {
  ...
  if (event.data.action === "check_brands") {
    console.log(
      "Received brands for lookup:",
      event?.data?.brands?.length || 0,
      "brands"
    );
  ...
  if (event.data.action === "response_brands") {
    console.log("Received brand data from another tab.");
    if (!storedNumbersSet) {
      storedNumbersSet = event.data.result;
      window.storedNumbersSet = storedNumbersSet;
      console.log(
        `Loaded ${storedNumbersSet.length} brand records from another tab.`
      );
    }
  }
};
...
(async () => {
  console.log("=== Brand Ingestion Script Starting ===");
